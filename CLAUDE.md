...
| Which pages have a view count at all, and why a page reads 0 views | the PAGEVIEW block in `main.js` - only `/blog/<slug>/`, the tools pages and the NINE static pages are counted - `/blog/` (the index) and `/tools/` (the hub) joined that list on 2026-09-22, because until then an arrival landing on either recorded a visit and no view, which made a real reader look identical to the read-nothing population. A page outside that allow-list reads 0 because nothing counts it, which is an ABSENCE, not a measurement | the `counters.page_views` table alone - a slug missing from it and a slug with 0 visitors look identical |
