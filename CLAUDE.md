...
| any link to getforgenta.com, or the nav/CTA templates in `scripts/publish-next.mjs` | `node scripts/tag-app-links.mjs` - exit 0 tagged, 1 untagged, 2 the instrument is broken. It checks BOTH halves: the committed HTML, and the GENERATOR, by rendering an article and asserting its links. The second half exists because a source scan cannot see tomorrow's post - without it the daily publish ships an untagged page and the gate notices the day after, once it is already live. `--fix` rewrites, `--limits` prints what it cannot see |
